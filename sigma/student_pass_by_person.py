...
import time
import random
"""http://sigma-test.17daxue.cn/web-sigma/app/teachercourse/getBookList?project=1&studentId=329705&moduleType=1"""
def get_book_list(student_id):
    ...
    return response
"""http://sigma-test.17daxue.cn/web-sigma/app/studentpass/getStudentPassList?studentId=329705&id=305&moduleType=1&taskType=4"""
def get_student_pass_list(student_id,book_id,module_type):
    ...
    return response
"""http://sigma-test.17daxue.cn/web-sigma/app/studentpass/getStudentPassContent?studentId=329705&taskId=467&moduleType=1&taskType=4&isNew=1"""
                        ...
                        if student_pass_info["status"] == 1:
                            for period_data in student_pass_info["periodData"]:
                                ...
                                if exercise_status == 0 or exercise_status == 2 or exercise_status == 3:
                                    ...
                                    task_content_rsp = get_student_pass_content(student_id,course_id,module_type,task_type)
...
if __name__ == "__main__":
    ...
    account_list = AccountMgr.get_account_list(root_dir+"/sigma/student_pass_by_person.csv")
    student_service = StudentService()
    sigma_service = SigmaService()
    for config_info in account_list:
        student_card = config_info[0]
        login_param = dict()
        login_param['loginName'] = student_card
        login_param['password'] = config_info[1]
        ...
        chapter_start = int(config_info[4] or 0)
        chapter_sum = int(config_info[5] or 0)
        pass_round = int(config_info[6] or 1)
        ...
        print("org_score=[%d],expect_add=[%d],expect_score=[%d]" % (init_score,add_scores,init_score+add_scores))
